...
test('win the first drill, see it marked, and move on', async ({ page }) => {
	...
	await expect(page.getByTestId('lessons-done')).toHaveText('1 of 6 done');
});
...
test("a guest's progress carries into their account and follows it to another device", async ({
	...
	await expect(phone.getByTestId('lessons-done')).toHaveText('0 of 6 done');
	...
	await phone.goto('/lessons');
	await expect(phone.getByTestId('lessons-done')).toHaveText('1 of 6 done');
