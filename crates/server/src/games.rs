...
//!
//! Persistence comes later; a restart forgets every game.
...
use crate::room::{Outgoing, Room, TimeControl};
...
impl Games {
    ...
    pub fn create(&self, time_control: TimeControl) -> CreatedGame {
        ...
        let entry = Arc::new(GameEntry {
            room: Mutex::new(Room::new(time_control)),
            tokens: tokens.clone(),
            tx,
        });
...
impl GameEntry {
    ...
    fn dispatch(&self, out: Vec<Outgoing>, reply: &mut Vec<ServerMessage>) {
        for o in out {
            match o {
...
async fn create_game(
    ...
    (StatusCode::CREATED, Json(games.create(tc)))
}

async fn snapshot(State(games): State<Games>, Path(id): Path<String>) -> Response {
    let Some(entry) = games.get(&id) else {
            ...
            incoming = stream.next() => {
                ...
                match serde_json::from_str::<ClientMessage>(&text) {
                    Ok(msg) => {
                        ...
                        entry.dispatch(out, &mut replies);
                        if is_move {
